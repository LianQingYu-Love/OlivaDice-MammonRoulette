...
class MLP:
    ...
    def train_step(self, x, target, lr):
        ...
        for layer in reversed(range(len(self.weights) - 1)):
            ...
            d_prev = deltas[-1]
            delta = [
                sum(w_next[k][j] * d_prev[k] for k in range(len(w_next))) * (1.0 if z[j] > 0.0 else 0.0) for j in range(len(z))
            ]
            deltas.append(delta)
        deltas.reverse()
...
class 斯蒂芬(AIComp, BaseAI):
    name = "斯蒂芬"
    brief = (
        "深度学习AI: 以深度Q网络(DQN)实时学习对局."
        "\n· 网络结构: 3层MLP(双隐层), 纯Python实现反向传播;"
        "\n· 学习方式: TD(0)时序差分 + 经验回放 + ε-贪心探索;"
        "\n· 模型持久化: 保存于配置的ai_model目录, 越打越强."
    )
    ...
    def __init__(self):
        ...
        sizes = [n_in, *self.HIDDEN, len(self.action_list)]
        self.net = MLP(sizes, self.rng)
        self.replay = ReplayBuffer(self.REPLAY_CAP)
        self.eps = self.EPS_START
        self.games = 0
        self._model_ready = False
        # 各对局轨迹: {game_id: {"state", "action", "info", "me"}}
        self._games = {}

    # region 模型持久化
    @property
    def model_path(self):
        return os.path.join(config.AI_MODEL_DIR, f"{self.name}.json")
    ...
    def decide(self, msg_manager) -> str:
        ...
        # 内联原 _choose 逻辑
        legal = [self.action_index[a] for a in AIComp.legal_actions(msg_manager) if a in self.action_index]
        if not legal:
            legal = [self.action_index["吞枪"]]
        q_values = self._q_all(state)
        if self.rng.random() < self.eps:
            action_idx = self.rng.choice(legal)
        else:
            action_idx = max(legal, key=lambda i: q_values[i])
    ...
    # region 学习
    def learning(self, msg_manager, action=None, user_id=None) -> None:
        """AI执行完一条指令后调用: 计算回报并训练网络."""
        game = msg_manager.val["game"]
        prev = self._games.pop(id(game), None)
        ...
        # 内联原 _reward 计算
        game, data, reply, tmp, modify, players, order, shooter, bullet = RegGameWork.get_index(msg_manager)
        me = user_id or prev.get("me") or shooter
        prev_hp = prev["info"]["hp"]
        cur_hp = {uid: players[uid]["hp"] for uid in players}
        prev_alive = set(prev["info"]["order"])
        cur_alive = set(order)
        ...
        # 内联原 _train 逻辑
        self.replay.push((prev["state"], prev["action"], reward, state_next, done))
